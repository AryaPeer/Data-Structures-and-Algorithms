...
bool BinarySearchTreeTest::test4() {
    ...
    BinarySearchTree::Node** root_ptr = bst.getRootNodeAddress();

    ASSERT_TRUE((*root_ptr)->val == 10)
    ASSERT_TRUE((*root_ptr)->val == bst.getRootNode()->val)
...
bool BinarySearchTreeTest::test5() {
    ...
    ASSERT_TRUE(bst.exists(5));
    ASSERT_TRUE(bst.exists(3));
    ASSERT_TRUE(bst.exists(7));
    ASSERT_TRUE(bst.exists(2));
    ASSERT_TRUE(bst.exists(4));
    ASSERT_TRUE(bst.exists(6));
    ASSERT_TRUE(bst.exists(8));
    ASSERT_TRUE(bst.exists(1));

    bst.remove(5);
    ASSERT_FALSE(bst.exists(5));

    bst.remove(3);
    ASSERT_FALSE(bst.exists(3));

    bst.remove(7);
    ASSERT_FALSE(bst.exists(7));

    bst.remove(2);
    ASSERT_FALSE(bst.exists(2));

    bst.remove(4);
    ASSERT_FALSE(bst.exists(4));

    bst.remove(6);
    ASSERT_FALSE(bst.exists(6));

    bst.remove(8);
    ASSERT_FALSE(bst.exists(8));
...
bool BinarySearchTreeTest::test8() {
    ...
    for (int i = 1; i <= 200; i++) {
        if (i >= 101) {
            ASSERT_TRUE(bst.exists(i));
        } else {
            ASSERT_FALSE(bst.exists(i));
        }
    }

    ASSERT_TRUE(bst.size()==100);
    return true;
}
